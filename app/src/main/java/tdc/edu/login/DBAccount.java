...
public class DBAccount extends SQLiteOpenHelper {
    ...
    public Collection<? extends Account> DocDLByAccountNameAndLevel(int levelaccount, String tenTk) {

        List<Account> accounts = new ArrayList<>();
        // get by cap do tai khoan
        String sql = "Select * from UserAccount where capdotaikhoan = ? and tentaikhoan like ?";
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery(sql, new String[]{String.valueOf(levelaccount), "%" + tenTk + "%"});
        if (cursor.moveToFirst()) {
            do {
                Account account = new Account();
                // Lấy chỉ số của cột
    ...
    public Collection<? extends Account> DocDLByAccountName(String tentk) {
        List<Account> accounts = new ArrayList<>();
        // get by cap do tai khoan
        String sql = "Select * from UserAccount where tentaikhoan like ?";
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.rawQuery(sql, new String[]{"%" + tentk + "%"});
        if (cursor.moveToFirst()) {
            do {
                Account account = new Account();
                // Lấy chỉ số của cột
